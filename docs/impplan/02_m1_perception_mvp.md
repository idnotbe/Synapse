...
| `synapse_a11y::click_element_action(&ElementId)` / `element_bounding_rect(&ElementId)` (Windows) | `crates/synapse-a11y/src/lib.rs` | M2 resolves and invokes element targets on the dedicated UIA MTA worker, returning only plain data to action callers |
| `synapse_a11y::snapshot_focused_window(depth)`, `snapshot_window_from_hwnd(hwnd, depth)`, `snapshot_element(id, depth)`, `foreground_context(hwnd)` | `crates/synapse-a11y/src/lib.rs` | M2/M1 uses data-returning UIA snapshots to record focused element state for manual before/after evidence |
| `uiautomation = "0.25.0"` re-exported as `synapse_a11y::UIElement` | `crates/synapse-a11y/src/lib.rs:15` | Kept for compatibility with legacy direct APIs; production callers use worker-owned data APIs so COM elements do not cross threads |
